...
public class Person {
	...
	private char gender;
	private String ssn;
	private List<String> validCTLDs = Arrays.asList("ad", "ae", "af", "ag", "ai", "al", "am", "ao", "aq", "ar", "as",
	...
	private void checkValidEmail(String email) {
		...
		if (!nameParts[0].equalsIgnoreCase(givenName)) {
			throw new IllegalArgumentException(
					"The first name of email (" + nameParts[0] + ") does not match given name (" + givenName + ")");
		}
		String familyName = this.name.substring(pos + 1);
		if (!nameParts[1].equalsIgnoreCase(familyName)) {
			throw new IllegalArgumentException(
					"The second name of email (" + nameParts[1] + ") does not match family name (" + familyName + ")");
		}
		...
		if (!(countryCodeLength == 2 || countryCodeLength == 3)) {
			throw new IllegalArgumentException(
					"The country code part of " + domainParts[1] + " must be 2 or 3 characters");
		}
		if (!isLettersOnly(domainParts[1], 0, -1)) {
			throw new IllegalArgumentException(
					"The country code part of " + domainParts[1] + " contains a non-letter character");
		}
