	...
	private void checkValidLetters(String regLetters) {
		if (!(Character.isUpperCase(regLetters.charAt(0)) && Character.isUpperCase(regLetters.charAt(1)))) {
			throw new IllegalArgumentException(
					"Registration numbers should start with two uppercase letters but was: " + regLetters);
		}
		if (fuelType == 'E' && !(regLetters.equals("EL") || regLetters.equals("EK"))) {
			throw new IllegalArgumentException(
					"For electrical vehicles the letters should be EL or EK but was :" + regLetters);
		}
		...
		if ((fuelType == 'D' || fuelType == 'G')
				&& (regLetters.equals("EL") || regLetters.equals("EK") || regLetters.equals("HY"))) {
			throw new IllegalArgumentException(
					"For diesel and petrol vehicles the letters should not be EL, EK or HY, but was: " + regLetters);
		}
